...
# Variable in Entry : ZVM server adress , Client ID (from keycloack) , Client sercret (from keycloack)
...
# Variables to Configure ot to pass as argument
if (($zvmAddress -eq "") -or ($null -eq $zvmAddress)) {
    $zvmAddress = "10.13.1.191"                                    # IP address or DNS name
    }
if (($keycloakClientID -eq "") -or ($null -eq $keycloakClientID)) {
    $keycloakClientID = "api-script"                              # defined in Keycloak - string name
    }
if (($keycloakClientSecret -eq "") -or ($null -eq $keycloakClientSecret)) {
...
$result | Select-Object VPGName,VMsCount, SourceSite, TargetSite, ActualRPO, vpgIdentifier, usedStorageinMb | format-table
